...
export interface DivergenceResult {
  ...
  change24h: number;               // Absolute 24H price difference
...
export interface SymbolData {
  ...
  divergence?: DivergenceResult; // NEW: Add divergence result to SymbolData
  // You might need these for PriceSnapshot:
  previousPrice?: number; // Price from a few minutes ago
  price24hAgo?: number; // Price from exactly 24 hours ago
}
...
export interface SentimentArticle {
  title: string;
  url: string;
  source: string; // Changed from source: { name: string } to string for simpler use
  publishedAt: string;
  content?: string;
...
export interface MarketData {
  ...
  totalLongLiquidationsUSD: number;
  totalShortLiquidationsUSD: number;
  // New properties for overall divergence summary
...
export interface MarketAnalysisResults {
  ...
  // New section for divergence analysis in MarketAnalysisResults
  divergenceAnalysis: MarketAnalysisResultDetail & {
    divergingCount: number;
    potentialFalseBreakoutCount: number;
...
export interface MarketStats {
  ...
  // Include aggregated divergence counts here for overall sentiment analysis
  divergingCount: number;
  potentialFalseBreakoutCount: number;
}
