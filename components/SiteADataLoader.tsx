...
const getSignal = (s: { rsi14?: number[] }): string => {
  ...
  const inRange = (val: number, min: number, max: number) => val !== undefined && val >= min && val <= max;
...
export default function SiteADataLoader() {
  ...
  useEffect(() => {
    ...
    const INTERVAL_MS = 500; // Decreased interval for faster loading
    let currentIndex = 0;
    let symbols: string[] = [];
    ...
    /**
     * Processes symbols one by one with a delay to display signals as they are found.
     */
    const processSymbolsIncrementally = async () => {
      if (!isMounted) return;
      ...
      // Process symbols one by one to update the UI incrementally
      const processNextSymbol = async (index: number) => {
        if (!isMounted || index >= symbols.length) {
          setLoading(false);
          return;
        }

        const symbol = symbols[index];
        const signal = await fetchAndAnalyze(symbol, timeframe);
        ...
        setTimeout(() => processNextSymbol(index + 1), INTERVAL_MS);
      };
  ...
  // Memoization and other logic remain the same
  ...
  const marketStats = useMemo(() => {
    const greenPriceChangeCount = signals.filter((t) => parseFloat(String(t.priceChangePercent)) > 0).length;
    const redPriceChangeCount = signals.filter((t) => parseFloat(String(t.priceChangePercent)) < 0).length;
    const greenVolumeCount = signals.filter((s) => s.highestVolumeColorPrev === 'green').length;
    const redVolumeCount = signals.filter((s) => s.highestVolumeColorPrev === 'red').length;
    const bullishTrendCount = signals.filter((s) => s.mainTrend && s.mainTrend.trend === 'bullish').length;
    const bearishTrendCount = signals.filter((s) => s.mainTrend && s.mainTrend.trend === 'bearish').length;
